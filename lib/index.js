...
function renderText(result) {
    ...
    const lines = result.results.map((m, i) => `${i + 1}. ${m.name} ★${m.stars}${m.curated ? ' [curated]' : ''} — ${m.description}\n   ${m.url}\n   install: ${m.install}`);
    return lines.join('\n\n') + (result.note ? `\n\n${result.note}` : '');
}
export function apply(ctx) {
    ctx.tools.register(defineTool({
        name: 'find_dsh_plugin',
        description: 'Search GitHub for DeepSeek Harness plugins (public `dsh-plugin` topic), ranked by stars. Returns ' +
        ...
        execute: async (args) => {
            ...
            try {
                ...
                for (const p of registry.plugins)
                    curated.set(p.url.toLowerCase(), p.description);
            }
